...
void AsterxDriverApp::QtThreadMain(asterx::AppConfig cfg) {
	...
	try {
		...
		Common::Log::log_and_throw(Common::Markers::kModuleAsterx, "AsteRx session error", e.what(), /*throw_error=*/false);
	}
...
bool AsterxDriverApp::init(const std::function<bool()> &external_stop) {
	...
	try {
		...
		Common::Log::log_and_throw(Common::Markers::kModuleAsterx, "AsteRx config error", e.what(), false);
		return false;
	}
	...
	if (outcome == BringUp::Recording) {
		Common::Log::log_message(spdlog::level::info, Common::Markers::kModuleAsterx, Common::Markers::kAsterxInitialized);
		return true;
	}
	if (outcome == BringUp::Failed) {
		...
		Common::Log::log_and_throw(Common::Markers::kModuleAsterx,
	...
	Common::Log::log_message(spdlog::level::warn, Common::Markers::kModuleAsterx, "AsteRx bring-up interrupted by shutdown request");
	return false;
}
...
void AsterxDriverApp::shutdown() {
	...
	Common::Log::log_message(spdlog::level::info, Common::Markers::kModuleAsterx, Common::Markers::kAsterxShutdown);
}
