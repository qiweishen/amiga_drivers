...
#                            -D_UNIX_ -D_LINUX_ + rpath link options)
...
foreach(_var EBUS_SDK_ROOT EBUS_ROOT)
    if(DEFINED ${_var} AND NOT "${${_var}}" STREQUAL "")
        list(APPEND _ebus_candidates "${${_var}}")
    endif()
    if(DEFINED ENV{${_var}} AND NOT "$ENV{${_var}}" STREQUAL "")
        list(APPEND _ebus_candidates "$ENV{${_var}}")
    endif()
endforeach()
...
foreach(_cand IN LISTS _ebus_candidates)
    if(EXISTS "${_cand}/include/PvSystem.h" AND EXISTS "${_cand}/include/PvDevice.h"
       AND EXISTS "${_cand}/lib/libPvBase.so")
...
if(NOT TARGET eBUS::eBUS)
    ...
    set_target_properties(eBUS::eBUS PROPERTIES
        ...
        # eBUS sample Makefiles compile with -D_UNIX_ -D_LINUX_ on Linux; some
        # SDK headers require them.
        INTERFACE_COMPILE_DEFINITIONS "_UNIX_;_LINUX_"
