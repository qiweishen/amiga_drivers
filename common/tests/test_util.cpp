...
TEST_CASE(
    ...
    CHECK(common::HumanBytes(512) == "512 B");
    CHECK(common::HumanBytes(1536) == "1.50 KiB");
    CHECK(common::HumanBytes(1299227607ull) == "1.21 GiB");
...
TEST_CASE(
    ...
    "Split keeps empties; SplitTrim uses getline semantics"
) {
    using namespace common::StringUtil;
    // Raw split (wire/CSV semantics): empties preserved, "" -> [""]
...
TEST_CASE(
    ...
    "ByteUtil round-trips and endianness"
) {
...
TEST_CASE(
    ...
    "BoundedQueue close-and-drain keeps tail items"
) {
...
TEST_CASE(
    ...
    "BoundedQueue capacity backpressure"
) {
...
TEST_CASE(
    ...
    "BoundedQueue push_blocking wakes a concurrent consumer"
) {
