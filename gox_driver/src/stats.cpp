...
namespace jai {
    ...
    std::string StatsReporter::periodic_line(double interval_s, uint64_t uptime_s, uint64_t free_disk_bytes) {
        ...
        snprintf(buf, sizeof(buf),
                 "[%s] up=%s fps=%.1f disk=%.1fMB/s ok=%" PRIu64 " incomp=%" PRIu64 " drop_q=%" PRIu64 " drop_net=%"
                 PRIu64
                 " gaps=%" PRIu64 "(-%" PRIu64 ") q=%" PRIu64 "/%" PRIu64 " seg=%" PRIu64 " written=%s free=%s",
                 camera_id_.c_str(), human_duration(uptime_s).c_str(), fps, mbps, cur.frames_retrieved_ok,
    ...
    std::string StatsReporter::final_summary(uint64_t uptime_s) const {
        ...
        snprintf(buf, sizeof(buf),
                 ...
                 " dropped_error=%" PRIu64 " blockid_gaps=%" PRIu64 " frames_lost=%" PRIu64 " stream_blocks_dropped=%"
                 PRIu64
