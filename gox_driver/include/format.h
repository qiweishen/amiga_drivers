...
// the first record at AlignUp(file_header_size, record_align), each next one
// at AlignUp(previous_record_end, record_align). record_align == 1 disables
...
// change requires a version bump here and in the readers that mirror this
// layout: scripts/inspect_raw.py, scripts/unpack_raw.py,
// scripts/dev/gen_test_segment.py and app/services/live_view.py.
...
namespace gox::format {
    ...
    FileHeader make_file_header(uint32_t segment_index, uint64_t created_realtime_ns, const uint8_t session_uuid[16],
                                const char *camera_id, const char *camera_serial, uint32_t record_align,
