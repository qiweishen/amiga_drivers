...
namespace LMS4xxx {
    ...
    struct ScanRecordWriter::Impl {
        ...
        [[nodiscard]] std::unique_ptr<Common::RotatingFileWriter> MakeFile() {
            ...
            opts.on_new_file = [this](std::ofstream &f) {
                ...
                g_log.trace("[{}] [Writer] Opened split file", config.instance);
                return true;
            };
            return std::make_unique<Common::RotatingFileWriter>(std::move(opts));
        }

        // Write thread main loop. Append() handles the size-based split (per-file header via on_new_file) internally
        void WriteLoop() {
            ...
            while (running.load(std::memory_order_acquire)) {
                ...
                while (count < kBatchSize && queue->try_pop(record)) {
                    if (!file->Append(record.data, record.size)) {
                        g_log.error("[{}] [Writer] Write failed on the split file — stopping the write loop",
                                    config.instance);
                        return;
                    }
            ...
            while (queue->try_pop(record)) {
                if (!file->Append(record.data, record.size)) {
                    g_log.error("[{}] [Writer] Write failed on the split file during drain", config.instance);
                    break;
                }
                stat_bytes_written.fetch_add(record.size, std::memory_order_relaxed);
                stat_frames_written.fetch_add(1, std::memory_order_relaxed);
    ...
    bool ScanRecordWriter::Start(const ScanConfig &scan_config) {
        ...
        if (!impl_->file->OpenNext()) {
            g_log.error("[{}] [Writer] Cannot open split file '{}'", impl_->config.instance,
                        impl_->GenerateFilePath(0));
            return false;
        }
        ...
        g_log.info(
            "[{}] [Writer] Recording to {} (record: {} B, channels: 0x{:02X}, queue: {} frames, max file: {} MB)",
            impl_->config.instance,
