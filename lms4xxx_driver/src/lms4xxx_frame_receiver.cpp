    ...
    struct FrameReceiver::Impl {
        ...
        std::size_t try_extract_frame(std::size_t pos) {
            ...
            if (data_len > max_frame_size) {
                ...
                g_log.warn("[{}] Frame data length {} exceeds max {}, skipping STX", tag, data_len, max_frame_size);
            ...
            if (computed_cs != received_cs) {
                ...
                g_log.warn("[{}] CRC8 mismatch: computed 0x{:02X}, received 0x{:02X}", tag, computed_cs, received_cs);
