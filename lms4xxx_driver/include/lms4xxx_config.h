...
namespace LMS4xxx {
    ...
    struct ScanConfig {
        ...
        [[nodiscard]] std::int32_t StartAngleDevice() const {
            return static_cast<std::int32_t>(start_angle_deg * 10000.0);
        }

        // Convert stop angle to device units (1/10000 degrees).
        [[nodiscard]] std::int32_t StopAngleDevice() const {
            return static_cast<std::int32_t>(stop_angle_deg * 10000.0);
        }
    ...
    struct NTPConfig {
        ...
        std::string server_ip;                  ///< no default: enabled NTP requires an explicit server
        std::uint32_t update_interval_s = 1;    ///< device valid range [1, 3600]
		std::uint32_t check_status_s = 5;	    ///< scanning-time server probe period, seconds [1, 3600]
    };
    ...
    struct DriverConfig {
        // Instance tag prefixed to every log line ("[front_left_laser] ...") so
        // multi-LiDAR runs stay attributable; empty = fall back to device.ip.
        std::string name;
