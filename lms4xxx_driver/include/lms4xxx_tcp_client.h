...
/// @brief Synchronous TCP client for the LMS4xxx CoLa B link (Boost.Asio
/// behind a Pimpl). Lived in common/ while the retired ins401 NTRIP client
/// shared it; lms4xxx is the sole user now.
///
/// The socket tuning and the shutdown sequencing are field-tested — keep the
/// semantics:
...
namespace LMS4xxx {
    ...
    // Map the LMS4xxx config structs onto the client options.
    [[nodiscard]] inline TCPClient::Options MakeTcpOptions(const DeviceConfig &device,
