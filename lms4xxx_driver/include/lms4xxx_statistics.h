...
namespace LMS4xxx {
    ...
    struct DriverStatistics {
        ...
        // NTP health as verified by probing the SERVER directly (SNTP query at
        // Configure() + a periodic watch while scanning). Deliberately not a
        // device-clock comparison: a previously-synced device keeps
        // near-correct time and would mask a dead server.
        enum NtpStatus : std::uint8_t {
            kNtpOff = 0, ///< ntp.enabled = false
            kNtpOk = 2, ///< server answered a healthy SNTP response
            kNtpNoTimestamp = 3, ///< device streams no timestamp block
            kNtpUnreachable = 4, ///< server stopped answering (device clock free-running)
        };

        std::atomic<std::uint8_t> ntp_status{kNtpOff};
        ...
        void Reset() {
            ...
            // ntp_configured_at_us and ntp_status are deliberately NOT reset:
            // they reflect configuration, not the session (Configure() runs
            // before StartScanning(), which calls Reset()).
        }
