...
namespace LMS4xxx {
    ...
    public:
        ...
        // `tag` is the owning instance's log-line prefix ("[front_left_laser] ...").
        [[nodiscard]] static std::error_code Parse(const std::uint8_t *data, std::size_t len, ScanData &out,
                                                   std::string_view tag = {});

    private:
        // Internal cursor for sequential binary field reading. Carries the
        // instance log tag so every parse warning stays attributable.
        struct Reader {
        ...
        [[nodiscard]] static ChannelContent16 IdentifyChannel16(const std::string &name, std::string_view tag);

        // Identify 8-bit channel content from 5-byte name string.
        [[nodiscard]] static ChannelContent8 IdentifyChannel8(const std::string &name, std::string_view tag);
    };
} // namespace LMS4xxx
