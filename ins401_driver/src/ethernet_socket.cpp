...
void EthernetSocket::CreateSocket() {
    ...
    if (socket_fd_ < 0) {
        if (errno == EPERM) {
            ...
            Tool::LogMessage(spdlog::level::err, kModule, __func__, "Failed to create raw socket",
                             std::strerror(errno));
        }
    }
    ...
    if (flags < 0 || fcntl(socket_fd_, F_SETFL, flags | O_NONBLOCK) < 0) {
        Tool::LogMessage(spdlog::level::warn, kModule, __func__, "Failed to set non-blocking mode",
                         std::strerror(errno));
    }
    ...
    if (ioctl(socket_fd_, SIOCGIFINDEX, &ifr) < 0) {
        Tool::LogMessage(spdlog::level::err, kModule, __func__,
                         fmt::format("Failed to get index for interface {}", interface_name_),
                         std::strerror(errno));
    }
    ...
    if (ioctl(socket_fd_, SIOCGIFHWADDR, &ifr) < 0) {
        Tool::LogMessage(spdlog::level::err, kModule, __func__,
                         fmt::format("Failed to get MAC for interface {}", interface_name_),
                         std::strerror(errno));
    }
    ...
    if (bind(socket_fd_, reinterpret_cast<sockaddr *>(&sll), sizeof(sll)) < 0) {
        Tool::LogMessage(spdlog::level::err, kModule, __func__,
                         fmt::format("Failed to bind socket to interface {}", interface_name_),
                         std::strerror(errno));
    }
...
void EthernetSocket::SetupEpoll() {
    ...
    if (epoll_fd_ < 0) {
        Tool::LogMessage(spdlog::level::err, kModule, __func__, "Failed to create epoll instance",
                         std::strerror(errno));
    }
...
namespace Ethernet {
    std::vector<std::pair<std::string, std::string> > GetNetworkInterfaces() {
        ...
        if (getifaddrs(&ifaddr) == -1) {
            Tool::LogMessage(spdlog::level::err, kModule, __func__, "Failed to get network interfaces",
                             std::strerror(errno));
        }
        ...
        if (fd < 0) {
            Tool::LogMessage(spdlog::level::err, kModule, __func__, "Failed to create socket for ioctl",
                             std::strerror(errno));
        }
        FdGuard fd_guard(fd);
    ...
    bool SetupBpfFilter(const MacAddress target_mac, const MacAddress local_mac, const int socket_fd) {
        ...
        if (setsockopt(socket_fd, SOL_SOCKET, SO_ATTACH_FILTER, &prog, sizeof(prog)) < 0) {
            Tool::LogMessage(spdlog::level::warn, kModule, __func__, "Failed to attach BPF filter",
                             std::strerror(errno));
        }
    ...
    std::array<uint8_t, 6> FormatMACAddress(const std::string_view mac_str) {
        ...
        if (hex_only.length() != 12) {
            Tool::LogMessage(
                ...
                fmt::format("Invalid MAC address length (expected 12 hex digits, got {}): '{}'", hex_only.length(),
                            mac_str));
        }
