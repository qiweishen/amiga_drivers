/**
 * @file ethernet_socket.cpp
 * @brief Implementation of EthernetSocket class.
 */
...
EthernetSocket::EthernetSocket(const std::string_view interface_name, const MacAddress &target_mac, const std::size_t recv_buffer_size,
							   const bool enable_bpf) :
	interface_name_(interface_name), target_mac_(target_mac), recv_buffer_size_(recv_buffer_size), enable_bpf_(enable_bpf) {
	CreateSocket();
	if (enable_bpf_) {
...
void EthernetSocket::CreateSocket() {
	...
	if (socket_fd_ < 0) {
		if (errno == EPERM) {
			...
			Tool::LogMessage(spdlog::level::err, kModule, __func__, "Failed to create raw socket", std::strerror(errno));
		}
	}
	...
	if (flags < 0 || fcntl(socket_fd_, F_SETFL, flags | O_NONBLOCK) < 0) {
		Tool::LogMessage(spdlog::level::warn, kModule, __func__, "Failed to set non-blocking mode", std::strerror(errno));
	}
	...
	if (ioctl(socket_fd_, SIOCGIFINDEX, &ifr) < 0) {
		Tool::LogMessage(spdlog::level::err, kModule, __func__, fmt::format("Failed to get index for interface {}", interface_name_),
						 std::strerror(errno));
	}
	if_index_ = ifr.ifr_ifindex;

	if (ioctl(socket_fd_, SIOCGIFHWADDR, &ifr) < 0) {
		Tool::LogMessage(spdlog::level::err, kModule, __func__, fmt::format("Failed to get MAC for interface {}", interface_name_),
						 std::strerror(errno));
	}
	...
	if (bind(socket_fd_, reinterpret_cast<sockaddr *>(&sll), sizeof(sll)) < 0) {
		Tool::LogMessage(spdlog::level::err, kModule, __func__, fmt::format("Failed to bind socket to interface {}", interface_name_),
						 std::strerror(errno));
	}
	...
	if (recv_buffer_size_ > 0) {
		...
		if (setsockopt(socket_fd_, SOL_SOCKET, SO_RCVBUF, &buf_size, sizeof(buf_size)) < 0) {
			Tool::LogMessage(spdlog::level::warn, kModule, __func__,
							 fmt::format("Failed to set receive buffer size on interface {}", interface_name_), std::strerror(errno));
		}
	}
}
...
void EthernetSocket::SetupEpoll() {
	...
	if (epoll_fd_ < 0) {
		Tool::LogMessage(spdlog::level::err, kModule, __func__, "Failed to create epoll instance", std::strerror(errno));
	}
	...
	if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, socket_fd_, &ev) < 0) {
		Tool::LogMessage(spdlog::level::err, kModule, __func__, "Failed to add socket to epoll", std::strerror(errno));
		close(epoll_fd_);
		epoll_fd_ = -1;
	}
}
...
namespace Ethernet {
	...
	std::vector<std::pair<std::string, std::string>> GetNetworkInterfaces() {
		...
		if (getifaddrs(&ifaddr) == -1) {
			Tool::LogMessage(spdlog::level::err, kModule, __func__, "Failed to get network interfaces", std::strerror(errno));
		}
		...
		if (fd < 0) {
			Tool::LogMessage(spdlog::level::err, kModule, __func__, "Failed to create socket for ioctl", std::strerror(errno));
		}
		FdGuard fd_guard(fd);
	...
	bool SetupBpfFilter(const MacAddress target_mac, const MacAddress local_mac, const int socket_fd) {
		...
		if (setsockopt(socket_fd, SOL_SOCKET, SO_ATTACH_FILTER, &prog, sizeof(prog)) < 0) {
			Tool::LogMessage(spdlog::level::warn, kModule, __func__, "Failed to attach BPF filter", std::strerror(errno));
		}
	...
	std::string ParseMacAddress(const uint8_t *mac_ptr) {
		if (!mac_ptr) {
			Tool::LogMessage(spdlog::level::err, kModule, __func__, "Null MAC address pointer");
			return "00:00:00:00:00:00";
		}
		return fmt::format("{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}", mac_ptr[0], mac_ptr[1], mac_ptr[2], mac_ptr[3], mac_ptr[4],
						   mac_ptr[5]);
	}
	...
	std::array<uint8_t, 6> FormatMACAddress(const std::string_view mac_str) {
		if (mac_str.empty()) {
			Tool::LogMessage(spdlog::level::err, kModule, __func__, "Empty MAC address string");
		}
		std::string hex_only;
		hex_only.reserve(12);
		for (char c: mac_str) {
		...
		if (hex_only.length() != 12) {
			Tool::LogMessage(
					spdlog::level::err, kModule, __func__,
					fmt::format("Invalid MAC address length (expected 12 hex digits, got {}): '{}'", hex_only.length(), mac_str));
		}
