...
bool NTRIPClient::ConnectSocket() {
	...
	if (ret != 0) {
		LogOrThrow(fmt::format("Failed to resolve hostname '{}': {}", config_.host, gai_strerror(ret)));
		return false;
	}
	...
	if (!local_connected) {
		LogOrThrow("Unable to connect to host '" + config_.host + "' on port " + port_str);
		return false;
	}
	return true;
}
...
bool NTRIPClient::InitSSL() {
	...
	if (flags == -1) {
		LogOrThrow("F_GETFL failed: " + std::string(strerror(errno)));
		return false;
	}
	...
	if (was_nonblock) {
		if (fcntl(socket_fd_, F_SETFL, flags & ~O_NONBLOCK) == -1) {
			LogOrThrow("Failed to set blocking mode: " + std::string(strerror(errno)));
			return false;
		}
	}
	...
	if (!ssl_ctx_) {
		LogOrThrow("Failed to create SSL context: " + GetSSLError());
		return false;
	}
	...
	if (config_.verify_ssl) {
		...
		if (SSL_CTX_set_default_verify_paths(ssl_ctx_.get()) != 1) {
			...
			if (!loaded) {
				LogOrThrow("Failed to load CA trust store from any known location");
				return false;
			}
		}
	} else {
	...
	if (!ssl_) {
		LogOrThrow("Failed to create SSL connection: " + GetSSLError());
		return false;
	}
	...
	if (SSL_set_fd(ssl_.get(), socket_fd_) != 1) {
		LogOrThrow("Failed to set SSL socket: " + GetSSLError());
		return false;
	}
...
bool NTRIPClient::SendRequest() const {
	...
	while (total_sent < request.size()) {
		...
		if (sent <= 0) {
			LogOrThrow("Failed to send request");
			return false;
		}
		total_sent += sent;
	}
...
bool NTRIPClient::ReceiveResponse() {
	...
	if (sel <= 0) {
		LogOrThrow((sel == 0) ? "Timeout waiting for response" : "Select error");
		CloseConnection();
		return false;
	}
	...
	while (response.find("\r\n\r\n") == std::string::npos) {
		...
		if (received <= 0) {
			LogOrThrow("Failed to receive response");
			return false;
		}
		buffer[received] = '\0';
		response += buffer;
	...
	if (header_part.find("200 OK") == std::string::npos && header_part.find("ICY 200 OK") == std::string::npos) {
		if (header_part.find("401") != std::string::npos) {
			LogOrThrow("Authentication failed");
		} else if (header_part.find("404") != std::string::npos) {
			LogOrThrow("Mount point not found: " + config_.mount_point);
		} else {
			LogOrThrow("HTTP error response");
		}
		return false;
	}
	return true;
		...
		while (receiving_.load(std::memory_order_acquire) && connected_.load(std::memory_order_acquire)) {
			...
			if (received > 0) {
				...
				if (errno != EAGAIN && errno != EWOULDBLOCK && errno != ETIMEDOUT) {
					connected_.store(false, std::memory_order_release);
					LogOrThrow("Receive error: " + std::string(strerror(errno)));
					if (config_.auto_reconnect && receiving_.load(std::memory_order_acquire)) {
						HandleReconnect();
					}
					break;
...
void NTRIPClient::HandleReconnect() {
	...
	// Reconnection failed
	LogOrThrow("Failed to reconnect after " + std::to_string(attempts) + " attempts");
	receiving_.store(false, std::memory_order_release);
}
	...
	if (ssl_) {
		...
		if (result <= 0) {
			...
			if (ssl_error != SSL_ERROR_WANT_READ && ssl_error != SSL_ERROR_WANT_WRITE) {
				LogOrThrow("SSL write error: " + std::to_string(ssl_error));
			}
		}
	} else {
		result = send(socket_fd_, data, size, MSG_NOSIGNAL);
		if (result < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
			LogOrThrow("Socket send error: " + std::string(strerror(errno)));
		}
	}
	return result;
}
...
ssize_t NTRIPClient::ReceiveData(void *buffer, size_t size) const {
	...
	if (ssl_) {
		...
		if (result <= 0) {
			int ssl_error = SSL_get_error(ssl_.get(), static_cast<int>(result));
			if (ssl_error != SSL_ERROR_WANT_READ && ssl_error != SSL_ERROR_WANT_WRITE) {
				LogOrThrow("SSL read error: " + std::to_string(ssl_error));
			}
		}
	} else {
		result = recv(socket_fd_, buffer, size, 0);
		if (result < 0 && errno != EAGAIN && errno != EWOULDBLOCK && errno != ETIMEDOUT) {
			LogOrThrow("Socket recv error: " + std::string(strerror(errno)));
		}
	}
	return result;
}
...
std::vector<NTRIPClient::MountPoint> NTRIPClient::GetSourceTable() {
	...
	while (std::getline(stream, line)) {
		if (line.find("STR;") == 0) {
			...
			std::vector<std::string> fields = InsTool::Utility::SplitString(line, ';');
...
void NTRIPClient::LogOrThrow(std::string_view msg) const {
