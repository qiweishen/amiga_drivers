...
StationaryDetector::StationaryDetector(const std::vector<RawIMUData> &raw_imu_data, double local_gravity,
                                       const Config &cfg)
...
bool StationaryDetector::IsStaticWindow(const ImuWindowStats &stats) const {
    ...
    return ((std::abs(gyro_mean.x()) < gyro_mean_threshold_xy_) && (
                std::abs(gyro_mean.y()) < gyro_mean_threshold_xy_) && (
                std::abs(gyro_mean.z()) < gyro_mean_threshold_z_)) && (gyro_std < gyro_var_threshold_) && (
               std::abs(accel_mean.norm() - local_gravity_) < accel_gravity_threshold_) && (
...
void StationaryDetector::AppendSegmentIfValid(size_t seg_start_idx, size_t seg_end_idx) {
    ...
    const double start_time = GpsWeekTowToSec(
        imu_data_[adjusted_start].gps_week, imu_data_[adjusted_start].gps_millisecs);
    const double end_time = GpsWeekTowToSec(
        imu_data_[adjusted_end].gps_week, imu_data_[adjusted_end].gps_millisecs);
...
void StationaryDetector::FindStationaryTimeSegments() {
    ...
    while (true) {
        ...
        if (window_end + step_samples_ >= imu_data_.size()) break;
...
void StationaryDetector::FindStationaryImuSegments() {
    ...
    for (const auto &[t_start, t_end]: stationary_time_segments_) {
        ...
        auto it_begin = std::lower_bound(
            imu_data_.begin(), imu_data_.end(), t_start,
            [](const ImuData &d, double t) {
                return GpsWeekTowToSec(d.gps_week, d.gps_millisecs) < t;
            });

        // Binary search for the first sample > t_end
        auto it_end = std::upper_bound(
            it_begin, imu_data_.end(), t_end,
            [](double t, const ImuData &d) {
                return t < GpsWeekTowToSec(d.gps_week, d.gps_millisecs);
            }
        );
