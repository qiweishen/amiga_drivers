	...
	if (detection_window_.size() < window_samples_) {
		if (detection_window_.size() % 100 == 0) {
			Common::Log::log_message(
					...
					fmt::format("Collecting data for 1# initialization ({}/{})", detection_window_.size(), window_samples_));
		}
		return;
	}
...
void InitializationMonitor::SetBlhFromGga(const Eigen::Vector3d &blh) {
	double gravity = INS401Tool::Earth::ComputeGravity(blh);
	std::scoped_lock lock(mutex_);
...
void InitializationMonitor::ComputeAndCheck(double current_time) {
	...
	const double stationary_duration = current_time - stationary_start_time_;
	Common::Log::log_message(spdlog::level::info, kModule,
							 fmt::format("Computation #{} at {:.1f}s stationary: roll={:.4f} deg pitch={:.4f} deg", stable_count_ + 1,
										 stationary_duration, alignment.roll / kDegToRad, alignment.pitch / kDegToRad));

	// Check stability
	if (CompareResults(result)) {
		stable_count_++;
		Common::Log::log_message(spdlog::level::info, kModule,
								 fmt::format("Static initialization computation {}/{}", stable_count_, config_.required_stable_count));
		if (has_position_) {
			Common::Log::log_message(spdlog::level::info, kModule,
									 fmt::format("GNSS status: position type {}; horizontal STD {}", latest_position_.position_type,
		...
		if (stable_count_ >= config_.required_stable_count) {
			if (!config_.enable_gnss_check || CheckGnssConditions()) {
				...
				Common::Log::log_message(
						spdlog::level::info, kModule,
						fmt::format("=== STATIC INITIALIZATION COMPLETE === : Gyro bias: [{:.6f}, {:.6f}, {:.6f}] deg/s",
									result.gyro_bias.x(), result.gyro_bias.y(), result.gyro_bias.z()));
				Common::Log::log_message(
						spdlog::level::info, kModule,
						fmt::format("=== STATIC INITIALIZATION COMPLETE === : Accel bias: [{:.6f}, {:.6f}, {:.6f}] m/s^2",
				...
				Common::Log::log_message(
						spdlog::level::info, kModule,
						fmt::format("=== STATIC INITIALIZATION COMPLETE === : Start time: {} week {} ms; End time: {} week {} ms",
				...
				if (!has_position_) {
					...
					Common::Log::log_message(spdlog::level::warn, kModule,
											 "=== Critical Warning === : If it is not on purpose, please check GNSS antenna, "
