...
void InitializationMonitor::OnImuData(const RawIMUData &raw_imu) {
	...
	if (detection_window_.size() < window_samples_) {
		if (detection_window_.size() % 100 == 0) {
			g_log.info("Collecting data for 1# initialization ({}/{})", detection_window_.size(), window_samples_);
		}
		return;
	}
	...
	if (IsStaticWindow()) {
		if (!is_stationary_) {
			is_stationary_ = true;
			stationary_start_time_ = GpsWeekTowToSec(detection_window_.front().gps_week, detection_window_.front().gps_millisecs);
			computation_buffer_.clear();
			computation_buffer_.assign(detection_window_.begin(), detection_window_.end());

			g_log.info("Stationary status detected");
		} else {
		...
		if (stationary_duration >= static_cast<double>(config_.min_stationary_duration_s)) {
			...
			if (!has_previous_result_ || time_since_last >= static_cast<double>(config_.recompute_interval_s)) {
				...
				g_log.info("Stationary for {:.0f} s, waiting {:.0f} s for next computation", stationary_duration,
						   config_.recompute_interval_s - time_since_last);
			}
		}
	} else {
		if (is_stationary_) {
			g_log.warn("Stationarity lost at GPS time {:.3f}s, resetting...", current_time);
			Reset();
		} else {
			g_log.warn("Not stationary for 1# initialization");
		}
	}
}
...
void InitializationMonitor::WaitForFirstGnssAndGravity(const std::chrono::milliseconds timeout) {
	...
	if (!success) {
		...
		gravity_ready_ = true;
		g_log.info("No GGA sentence received, proceeding with default gravity value of 9.8 m/s^2");
	} else {
		g_log.info("Gravity computed from GGA coordinates: {:.6f} m/s^2", config_.local_gravity);
	}
}
...
bool InitializationMonitor::IsStaticWindow() const {
	...
	const double accel_std = std::sqrt(std::max(0.0, window_stats_.accel_sq_sum * inv_n - accel_mean.squaredNorm()));
	const double gyro_std = std::sqrt(std::max(0.0, window_stats_.gyro_sq_sum * inv_n - gyro_mean.squaredNorm()));

	return (std::abs(gyro_mean.x()) < config_.gyro_mean_threshold_xy) && (std::abs(gyro_mean.y()) < config_.gyro_mean_threshold_xy) &&
		   (std::abs(gyro_mean.z()) < config_.gyro_mean_threshold_z) && (gyro_std < config_.gyro_var_threshold) &&
...
void InitializationMonitor::ComputeAndCheck(double current_time) {
	...
	if (has_position_) {
		result.position = Eigen::Vector3d(latest_position_.latitude, latest_position_.longitude, latest_position_.height);
	}

	const double stationary_duration = current_time - stationary_start_time_;
	g_log.info("Computation #{} at {:.1f}s stationary: roll={:.4f} deg pitch={:.4f} deg", stable_count_ + 1, stationary_duration,
			   alignment.roll / kDegToRad, alignment.pitch / kDegToRad);

	if (CompareResults(result)) {
		stable_count_++;
		g_log.info("Static initialization computation {}/{}", stable_count_, config_.required_stable_count);
		if (has_position_) {
			g_log.info("GNSS status: position type {}; horizontal STD {}", latest_position_.position_type,
		...
		if (stable_count_ >= config_.required_stable_count) {
			if (!config_.enable_gnss_check || CheckGnssConditions()) {
				...
				g_log.info("=== STATIC INITIALIZATION COMPLETE === : Roll: {:.4f} deg; Pitch: {:.4f} deg", result.roll / kDegToRad,
						   result.pitch / kDegToRad);
				g_log.info("=== STATIC INITIALIZATION COMPLETE === : Gyro bias: [{:.6f}, {:.6f}, {:.6f}] deg/s", result.gyro_bias.x(),
						   result.gyro_bias.y(), result.gyro_bias.z());
				g_log.info("=== STATIC INITIALIZATION COMPLETE === : Accel bias: [{:.6f}, {:.6f}, {:.6f}] m/s^2", result.accel_bias.x(),
						   result.accel_bias.y(), result.accel_bias.z());
				g_log.info("=== STATIC INITIALIZATION COMPLETE === : GNSS position std: {:.6f} m",
						   (latest_position_.latitude_std + latest_position_.longitude_std) / 2.0f);
				g_log.info("=== STATIC INITIALIZATION COMPLETE === : Start time: {} week {} ms; End time: {} week {} ms",
				...
				if (!has_position_) {
					g_log.warn("=== Critical Warning === : Not receiving any GNSS data during the whole static initialization");
					g_log.warn("=== Critical Warning === : If it is not on purpose, please check GNSS antenna, "
							   "connection and configuration");
				}
			} else {
				g_log.warn("Stability reached but GNSS conditions not met. Waiting for fresh RTK_FIXED and low std");
			}
		}
	} else {
		g_log.warn("Result unstable (roll delta={:.4f} deg, pitch delta={:.4f} deg), "
				   "Reset stable count",
				   std::abs(result.roll - last_result_.roll) / kDegToRad, std::abs(result.pitch - last_result_.pitch) / kDegToRad);
...
bool InitializationMonitor::CheckGnssConditions() const {
	...
	const double position_std =
			(static_cast<double>(latest_position_.latitude_std) + static_cast<double>(latest_position_.longitude_std)) / 2.0;
