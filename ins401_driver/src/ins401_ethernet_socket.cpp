...
void EthernetSocket::CreateSocket() {
	...
	if (ioctl(socket_fd_, SIOCGIFINDEX, &ifr) < 0) {
		Common::Log::log_and_throw(kModule, fmt::format("Failed to get index for interface {}", interface_name_),
								   std::strerror(errno));
	}
	if_index_ = ifr.ifr_ifindex;

	if (ioctl(socket_fd_, SIOCGIFHWADDR, &ifr) < 0) {
		Common::Log::log_and_throw(kModule, fmt::format("Failed to get MAC for interface {}", interface_name_), std::strerror(errno));
	}
	...
	if (bind(socket_fd_, reinterpret_cast<sockaddr *>(&sll), sizeof(sll)) < 0) {
		Common::Log::log_and_throw(kModule, fmt::format("Failed to bind socket to interface {}", interface_name_),
								   std::strerror(errno));
	}
	...
	std::array<uint8_t, 6> FormatMACAddress(std::string mac_str) {
		...
		for (char c: mac_str) {
			if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F')) {
				...
				Common::Log::log_and_throw(kModule, fmt::format("Invalid character '{}' in MAC address: '{}'", c, mac_str));
			}
		}
		if (hex_only.length() != 12) {
			Common::Log::log_and_throw(kModule, fmt::format("Invalid MAC address length (expected 12 hex digits, got {}): '{}'",
