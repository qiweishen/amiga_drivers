...
INSDeviceReceiver::INSDeviceReceiver(const std::string &iface,
                                     const std::string &mac_addr,
                                     bool save_to_file)
    : sock_fd_(-1), interface_name_(std::move(iface)), running_(true),
...
void INSDeviceReceiver::Stop() { running_ = false; }
...
bool INSDeviceReceiver::GetGNSSData(std::vector<GNSSSolutionData> &data,
                                    size_t max_count) {
...
bool INSDeviceReceiver::GetIMUData(std::vector<RawIMUData> &data,
                                   size_t max_count) {
  std::lock_guard lock(queue_mutex_);
  if (imu_queue_.empty()) {
    return false;
...
void INSDeviceReceiver::ProcessGNSSSolutionData(const uint8_t *packet) {
  ...
  constexpr size_t crc_offset =
      ACENINNA_HEADER_LEN + GNSS_SOLUTION_PACKET_LENGTH;
  const uint16_t recv_crc =
      (packet[crc_offset] << 8) | packet[crc_offset + 1]; // MSB-first
  if (const uint16_t calc_crc =
          Tool::CRC::CalculateINS401_CRC16(&packet[2], 2 + 4 + data_length);
      recv_crc != calc_crc) {
    std::cerr << "GNSS solution data CRC mismatch! Received: 0x" << std::hex
              << recv_crc << " Calculated: 0x" << calc_crc << std::dec
              << std::endl;
    return;
  }
  ...
  gnss.gps_millisecs = gnss_solution_data[2] | (gnss_solution_data[3] << 8) |
                       (gnss_solution_data[4] << 16) |
                       (gnss_solution_data[5] << 24);
...
void INSDeviceReceiver::ProcessRawIMUData(const uint8_t *packet) {
  ...
  // Check CRC
  constexpr size_t crc_offset =
      ACENINNA_HEADER_LEN + RAW_IMU_DATA_PACKET_LENGTH;
  const uint16_t recv_crc =
      (packet[crc_offset] << 8) | packet[crc_offset + 1]; // MSB-first
  if (const uint16_t calc_crc =
          Tool::CRC::CalculateINS401_CRC16(&packet[2], 2 + 4 + data_length);
      recv_crc != calc_crc) {
    std::cerr << "Raw IMU data CRC mismatch! Received: 0x" << std::hex
              << recv_crc << " Calculated: 0x" << calc_crc << std::dec
              << std::endl;
    return;
  }
  ...
  imu.gps_millisecs = imu_data[2] | (imu_data[3] << 8) | (imu_data[4] << 16) |
                      (imu_data[5] << 24);
  ...
  while (running_) {
    ...
    if (save_to_file_ && imu_file_.is_open() && !imu_batch.empty()) {
      for (const auto &imu : imu_batch) {
        imu_file_ << imu.gps_week << "," << imu.gps_millisecs << ","
    ...
    if (save_to_file_ && gnss_file_.is_open() && !gnss_batch.empty()) {
      for (const auto &gnss : gnss_batch) {
        gnss_file_ << gnss.gps_week << "," << gnss.gps_millisecs << ","
                   << static_cast<int>(gnss.position_type) << "," << std::fixed
                   << std::setprecision(9) << gnss.latitude << ","
                   << gnss.longitude << "," << std::setprecision(3)
                   << gnss.height << "," << gnss.latitude_std << ","
                   << gnss.longitude_std << "," << gnss.height_std << ","
                   << static_cast<int>(gnss.num_of_SVs) << ","
                   << static_cast<int>(gnss.num_of_SVs_in_solution) << ","
                   << gnss.hdop << "," << gnss.diffage << "," << gnss.north_vel
                   << "," << gnss.east_vel << "," << gnss.up_vel << ","
                   << gnss.north_vel_std << "," << gnss.east_vel_std << ","
                   << gnss.up_vel_std << "\n";
      }
      gnss_file_.flush();
    }
