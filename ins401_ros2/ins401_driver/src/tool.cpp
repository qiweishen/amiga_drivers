...
namespace Tool {
	namespace Ethernet {
		...
		void ParseMACAddressToUint8(const std::string &mac_str, std::array<uint8_t, 6> &mac_uint8) {
			...
			int result = std::sscanf(mac_str.c_str(),
		...
		bool CreateAsyncRawSocket(int &raw_socket, const std::string &interface, size_t buffer_size) {
		    ...
		    if (raw_socket < 0) {
		        ...
		        if (errno == EPERM) {
		            std::cerr << "Note: This program requires root privileges (sudo)"
		                      << std::endl;
		        }
		        return false;
		    }
		    ...
		    if (ioctl(raw_socket, SIOCGIFINDEX, &ifr) < 0) {
		        std::cerr << "Error: Failed to get interface index for "
		    ...
		    if (bind(raw_socket, reinterpret_cast<sockaddr*>(&sll),
		             sizeof(sll)) < 0) {
		    ...
		    if (buffer_size > 0) {
		        if (setsockopt(raw_socket, SOL_SOCKET, SO_RCVBUF,
		                      &buffer_size, sizeof(buffer_size)) < 0) {
		            std::cerr << "Warning: Failed to set receive buffer size - "
		                      << strerror(errno) << std::endl;
		        }
		    }
		    ...
		    if (setsockopt(raw_socket, SOL_PACKET, PACKET_FANOUT,
		...
		bool SetupEpollForFd(int sock_fd, int &epfd_out, const uint32_t events) {
			...
			if (epoll_ctl(epfd_out, EPOLL_CTL_ADD, sock_fd, &ev) < 0) {
				std::cerr << "Error: epoll_ctl ADD failed for fd=" << sock_fd
						  << " : " << strerror(errno) << std::endl;
				close(epfd_out);
				epfd_out = -1;
				return false;
